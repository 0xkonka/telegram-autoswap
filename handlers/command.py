...
from utils.balance import get_gas_balance , get_trade_token_balance
...
def command_handler(bot: Client):
    ...
    async def start(client: Client, message: Message):
        ...
        try:
            ...
            if wallet.evm_wallet_address:
                ...
                await message.reply(f"{START_MESSAGE}", reply_markup=InlineKeyboardMarkup(buttons))
            else:
                buttons = [
    ...
    async def balance_trade(client: Client, message: Message):
        ...
        user_id = message.from_user.id
        wallet = await get_wallet(user_id)
        balance_message = await get_trade_token_balance(wallet)
        await message.reply(balance_message)
