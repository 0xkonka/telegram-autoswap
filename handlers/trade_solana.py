...
def solana_trade_handler(bot: Client):
    ...
    async def on_auto_trade_solana(client: Client, callback_query: CallbackQuery):
        ...
        try:
            ...
            amount_states[user_id] = "awaiting_solana_amount"
        except Exception as e:
            logger.error(f"Error get trading amount: {e}")

    @bot.on_message(filters.text & filters.create(lambda _, __, msg: amount_states.get(msg.from_user.id) == "awaiting_solana_amount"))
